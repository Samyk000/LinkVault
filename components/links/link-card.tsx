...
function LinkCardComponent({ link, isInTrash = false, isSelected = false, onToggleSelect, isSelectionModeActive = false, priority = false }: LinkCardProps) {
  ...
  return (
    <>
      <Card
        className={`group relative overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-primary/5 cursor-pointer border-2 rounded-xl will-change-transform ${
          isSelected 
      ...
      <div className="relative h-28 sm:h-32 w-full overflow-hidden bg-muted/50 rounded-t-xl">
        ...
        {!isInTrash && (
          <div className={`absolute top-2.5 left-2.5 z-10 transition-all duration-200 ${
